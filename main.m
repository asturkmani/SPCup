    ...
    for i = 1:2
        ...
        while(temp<sizeT)
            if(sizeT-temp>=360000)
             ...
             [result_freq_vs_time] = extract_ENF(recording_signal(temp:(temp-1+360000)), 1, moving_median_width, Fs, ...
                                 Time_Step, Percent_Overlap, Padding_Factor, filter_half_size);
            
            else
             [result_freq_vs_time] = extract_ENF(recording_signal((sizeT-360000):sizeT), 1, moving_median_width, Fs, ...
                                 Time_Step, Percent_Overlap, Padding_Factor, filter_half_size);
